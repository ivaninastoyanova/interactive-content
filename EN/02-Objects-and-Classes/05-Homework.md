...
Each element holds data of a hero in the following format: 
...
[/hint] 
[hint]
Last, we need to convert the array of objects into a JSON string, which is done with the `JSON.stringify()` method.
...
Each element is a JSON string that represents the data of a certain employee. 
